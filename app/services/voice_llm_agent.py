...
from app.config import SOIL_THRESHOLDS  # ← centralized thresholds
...
def get_crop_recommendation_context(
    ...
    """
    Build a crop-recommendation-specific context string injected into the prompt.
    Uses SOIL_THRESHOLDS from app.config for all nutrient comparisons.
    Uses agronomic base doses so fertilizer amounts are never zero.
    """
    n    = sensor_data.get('nitrogen',    0)
    p    = sensor_data.get('phosphorus',  0)
    ...
    # Pull thresholds from centralized config
    n_min  = SOIL_THRESHOLDS["N"]["min"]
    n_max  = SOIL_THRESHOLDS["N"]["max"]
    p_min  = SOIL_THRESHOLDS["P"]["min"]
    p_max  = SOIL_THRESHOLDS["P"]["max"]
    k_min  = SOIL_THRESHOLDS["K"]["min"]
    k_max  = SOIL_THRESHOLDS["K"]["max"]
    ph_min = SOIL_THRESHOLDS["pH"]["min"]
    ph_max = SOIL_THRESHOLDS["pH"]["max"]

    # Status labels using centralized thresholds
    n_s  = "low"     if n  < n_min  else "high"     if n  > n_max  else "good"
    p_s  = "low"     if p  < p_min  else "high"     if p  > p_max  else "good"
    k_s  = "low"     if k  < k_min  else "high"     if k  > k_max  else "good"
    ph_s = "acidic"  if ph < ph_min else "alkaline" if ph > ph_max else "neutral/good"

    # Best 3 crops based on soil profile (using centralized pH thresholds)
    best1 = "Wheat"     if ph_min <= ph <= ph_max else "Maize"
    best2 = "Potato"    if k >= k_min             else "Onion"
    best3 = "Mustard"   if temp <= 25             else "Sunflower"
    ...
    c1 = (dose(BASE_UREA, best1, n, n_min, n_max), dose(BASE_DAP, best1, p, p_min, p_max), dose(BASE_MOP, best1, k, k_min, k_max))
    c2 = (dose(BASE_UREA, best2, n, n_min, n_max), dose(BASE_DAP, best2, p, p_min, p_max), dose(BASE_MOP, best2, k, k_min, k_max))
    c3 = (dose(BASE_UREA, best3, n, n_min, n_max), dose(BASE_DAP, best3, p, p_min, p_max), dose(BASE_MOP, best3, k, k_min, k_max))
    ...
    if language == "ur-PK":
        ...
        avoid = "چاول" if ph > ph_max or hum < 40 else "کپاس" if ph < ph_min else "گنا"
        context = f"""
فصل کی سفارش کے لیے اضافی تجزیہ:
حد کی اقدار: نائٹروجن ({n_min} سے {n_max} ملی گرام فی کلوگرام)، فاسفورس ({p_min} سے {p_max} ملی گرام فی کلوگرام)، پوٹاشیم ({k_min} سے {k_max} ملی گرام فی کلوگرام)، پی ایچ ({ph_min} سے {ph_max})
بہترین پہلی فصل: {best1} — یوریا {c1[0]} کلوگرام، ڈی اے پی {c1[1]} کلوگرام، ایم او پی {c1[2]} کلوگرام ({land_size} ایکڑ کے لیے)
بہترین دوسری فصل: {best2} — یوریا {c2[0]} کلوگرام، ڈی اے پی {c2[1]} کلوگرام، ایم او پی {c2[2]} کلوگرام
بہترین تیسری فصل: {best3} — یوریا {c3[0]} کلوگرام، ڈی اے پی {c3[1]} کلوگرام، ایم او پی {c3[2]} کلوگرام
جس فصل سے بچیں: {avoid}
آبپاشی: {water_ur}
نائٹروجن حالت: {n_s} ({n} ملی گرام فی کلوگرام، حد {n_min} سے {n_max})
فاسفورس حالت: {p_s} ({p} ملی گرام فی کلوگرام، حد {p_min} سے {p_max})
پوٹاشیم حالت: {k_s} ({k} ملی گرام فی کلوگرام، حد {k_min} سے {k_max})
پی ایچ حالت: {ph_s} ({ph}، حد {ph_min} سے {ph_max})
ان اعداد و شمار کی بنیاد پر وائس کے لیے مناسب جواب دیں۔ اعداد اردو میں لکھیں اور بولنے میں فطری لگیں۔
"""
    else:
        avoid = "Rice" if ph > ph_max or hum < 40 else "Cotton" if ph < ph_min else "Sugarcane"
        context = f"""
CROP RECOMMENDATION ANALYSIS:
Threshold ranges used: N ({n_min}–{n_max} mg/kg), P ({p_min}–{p_max} mg/kg), K ({k_min}–{k_max} mg/kg), pH ({ph_min}–{ph_max})
Best 1st crop: {best1} — Urea {c1[0]} kg, DAP {c1[1]} kg, MOP {c1[2]} kg (for {land_size} acres)
Best 2nd crop: {best2} — Urea {c2[0]} kg, DAP {c2[1]} kg, MOP {c2[2]} kg
Best 3rd crop: {best3} — Urea {c3[0]} kg, DAP {c3[1]} kg, MOP {c3[2]} kg
Crop to avoid: {avoid}
Watering: {water_advice}
Nitrogen status: {n_s} ({n} mg/kg, threshold {n_min}–{n_max})
Phosphorus status: {p_s} ({p} mg/kg, threshold {p_min}–{p_max})
Potassium status: {k_s} ({k} mg/kg, threshold {k_min}–{k_max})
pH status: {ph_s} ({ph}, threshold {ph_min}–{ph_max})
Use these calculated values in your voice response. Speak naturally without any special formatting.
"""
    return context
