...
// 1. Added optional `idealRange` prop to the interface
interface MetricCardProps {
  icon: LucideIcon;
  label: string;
  value: number;
...
export function MetricCard({
  ...
  color,
  index,
  idealRange, // 1. Destructured from props
}: MetricCardProps) {
  const { t } = useLanguage();

  // 2. EC Unit Conversion Logic
  // Detects if the metric is EC/Conductivity-based and converts µS/cm → mS/cm
  const isEC = label.includes("EC") || label.includes("Conductivity");
  const displayValue = isEC ? (value / 1000).toFixed(2) : value;
  const displayUnit = isEC ? "mS/cm" : unit;
    ...
    >
      ...
      <div className="flex items-baseline gap-2">
        <motion.span
          ...
          className="text-3xl font-bold text-foreground tabular-nums"
        >
          {/* 2. Renders converted value (÷1000 + toFixed(2)) for EC/Conductivity labels */}
          {displayValue}
        </motion.span>
        {/* 2. Renders 'mS/cm' for EC/Conductivity labels, otherwise the passed `unit` */}
        <span className="text-sm text-muted-foreground">{displayUnit}</span>
      </div>

      {/* 3. Footer: status badge + optional idealRange pill */}
      <div className="mt-3 flex items-center gap-4 flex-wrap">
        ...
        {/* 3. Conditionally rendered — only appears when `idealRange` is provided */}
        {idealRange && (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-foreground">
            Ideal: {idealRange}
          </span>
